...
/*
 * This program uses analytic expression of the test-function in 3D 
 * and computes its integrals along the set of rays in 3D. The computations are performed
...
void generate_data_readme(char* filename, int Nz, int nshift, int nphi) {
  FILE *doc_file;
  ...
  fprintf(doc_file, "  Ray transforms in 3D for the test-function given in %s\n", filename);
