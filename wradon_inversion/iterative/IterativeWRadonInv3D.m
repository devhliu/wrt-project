...
% normalization of weights (w_{k,n} / w_{0,0}), precomputation of all harmonics
  ...
  cutoff = GenCutoffFunc(ngrid);
                                                                      
  u_new = zeros(ngrid, ngrid, ngrid);  % two layer iterative scheme; next function layer
  u_old = F0 .* weights(:, :, :, 1);                          % current function layer
  u0 = F0 .* weights(:, :, :, 1);
...
% ------------------------ Cycle of iterations ---------------------------------
  do 
    u_new = u0 - IntOperatorQ(ngrid, degree, weights_normalized, harmonics, cutoff, u_old);
    u_old = u_new;
