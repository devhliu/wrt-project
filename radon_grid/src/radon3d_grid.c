 ...
 * and computes integrals along a set of 2d-planes, where this set corresponds to
 * 'Gauss-uniform' sampling scheme. The parameters of this scheme 
 * lie in a separate configuration file (-p filename).
 * 
...
int main(int argc, char * argv[]) {
  ...
  double*** values;
  if (init_values_alloc(ngrid, &values) != 0) {
     perror("Aborting : init_values_alloc.\n");
     exit(EXIT_FAILURE);
  }
  if (init_values(input_filename, ngrid, values) != 0) {
  ...
  if (clean_values(ngrid, values) != 0) {
