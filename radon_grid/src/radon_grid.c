 ...
 * is described by it values on the uniform grid in [-1,1]^3 which are stored in a separate csv file. 
 ...
 * 
 * For integration along planes the zero order (Riemann) rule is used.  
 *
 ...
 * ***************************************************************************
 */
void readme_generate(char* output_filename, int ngrid, int nphi, int ntheta, int nshift) {
  FILE *doc_file;
  ...
  fprintf(doc_file, "    number of grid ponts [ngrid]: %d,\n", ngrid);
...
void print_usage(FILE* stream, int exit_code) {
  ...
  fprintf(stream, 
	  ...
	  "   -i --input filename       Read grid data from file according to parameters that have been read.\n"
...
int main(int argc, char * argv[]) {
  ...
  if (parameters_alloc(&parameters) != 0) {
     perror("Aborting : parameters_alloc failed.\n");
     exit(EXIT_FAILURE);
  }
  if (parameters_init(parameters_filename, parameters) != 0) {
     perror("Aborting : parameters_init failed.\n");
     exit(EXIT_FAILURE);
  }
  printf("Done.\n");
        ...
        for (i_shift = i_shift_min; i_shift < i_shift_max; ++i_shift) {      
            ...
            for (i_phi = 0; i_phi < nphi; ++i_phi) {
	              ...
	              for (i_theta = 0; i_theta < ntheta; ++i_theta) {
		               //plane integral
		               double rt = plane_integral(values, ngrid, 
  ...
  readme_generate(output_filename, ngrid, nphi, ntheta, nshift);
  printf("  Done.\n");
  ...
  if (tfunction_values_clean(ngrid, values) != 0) {
